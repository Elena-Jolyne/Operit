...
exports.renderPlanExecution = renderPlanExecution;
exports.parsePlanExecution = parsePlanExecution;
...
function parsePlanStream(content) {
    ...
    if (graphMatch && graphMatch[1]) {
        try {
            graph = (0, PlanParser_1.parseExecutionGraph)(graphMatch[1]);
        }
        catch (e) {
    ...
    return {
        ...
        error: errorMatch && errorMatch[1] ? errorMatch[1] : null
    };
}
// Matches Kotlin's PlanExecutionRenderer @Composable function (lines 136-212)
function renderPlanExecution(ctx, content) {
    const { UI } = ctx;
    const state = parsePlanStream(content || "");
    if (state.graph) {
        return renderExecutionGraphDisplay(ctx, state.graph, state.taskStatuses, state.taskToolCounts, state.logs, state.summary);
    }
...
function renderExecutionGraphDisplay(ctx, graph, taskStatuses, taskToolCounts, logs, summary) {
    const { UI } = ctx;
    const children = [];
    const i18n = getI18n();
    ...
    if (logs.length > 0) {
        ...
        children.push(UI.Column({ height: 36, spacing: 2 }, recentLogs.map(log => UI.Text({
...
function renderWorkflowGraph(ctx, graph, taskStatuses, taskToolCounts) {
    const { UI } = ctx;
    ...
    let sortedTasks;
    try {
        sortedTasks = (0, PlanParser_1.topologicalSort)(graph);
    }
    catch (e) {
    ...
    nodePositions.forEach(nodeData => {
        ...
        commands.push(...drawTaskNode(ctx, nodeData, status, toolCount, todoColor, inProgressColor, completedColor, failedColor));
    });
    return UI.Canvas({
        commands,
        fillMaxSize: true,
...
function calculateNodePositions(sortedTasks, graph, canvasSize) {
...
function drawTaskNode(ctx, nodeData, status, toolCount, todoColor, inProgressColor, completedColor, failedColor) {
    const commands = [];
    const { taskNode: task, position, size } = nodeData;
    const cornerRadius = 8;
    // Determine accent color - matches lines 559-564
    const accentColor = status === "TODO"
        ? todoColor
        : status === "IN_PROGRESS"
            ? inProgressColor
            : status === "COMPLETED"
    ...
    const instructionTop = padding + taskNameLayout.height + taskIdLayout.height + toolCountLayout.height + 6;
...
function parsePlanExecution(content) {
